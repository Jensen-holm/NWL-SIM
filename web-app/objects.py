...
class NWLHitter():
    ...
    def rate_stats(self):
            ...
            st.text(f'{self.Name} {self.hand}: PA: {self.PA} AVG: {self.AVG:.3f} OBP: {self.OBP:.3f} SLG: {self.SLG:.3f} OPS: {self.OPS:.3f} HR: {self.HR} RBI: {self.RBI}')
...
class NWLPitcher():
    ...
    def rate_stats(self):
        if self.IP != 0:
            ...
            st.text(f'{self.Name} {self.hand}: WHIP: {self.WHIP:.2f} ERA: {self.ERA:.2f} K/9: {self.K_9:.2f}')
    ...
    def set_lineup(self):
        if self.lineup_settings == 'manual':
            ...
            for player in self.hitters:
                st.text(str(player.Name) + ' ' + str(player.hand) + ' ' +  str(player.status))
